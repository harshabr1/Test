                ...
                if (appDoc.RecordTypeId == recordTypeId && appDoc.Status_vod__c == 'Approved_vod') {
                    ...
                    if (vaultDocId != null && vaultInstance != null) {
                        List<Approved_Document_vod__c> oldDocs = 
                            new List<Approved_Document_vod__c>([SELECT RecordTypeId, Name, Vault_Document_ID_vod__c,  Id, Status_vod__c, Email_Subject_vod__c, Vault_Instance_ID_vod__c, Document_ID_vod__c FROM Approved_Document_vod__c Where RecordTypeId = :recordTypeId AND Vault_Instance_ID_vod__c = :vaultInstance AND Vault_Document_ID_vod__c = :vaultDocId]);
                        if (!oldDocs.isEmpty()) {
