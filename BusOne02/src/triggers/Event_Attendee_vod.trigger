trigger Event_Attendee_vod on Event_Attendee_vod__c (before update, before insert) {   
    ...
    for (Integer i=0; i < Trigger.new.size(); i++) {
        ...
        if (Trigger.new[i].Child_Account_vod__c != null) {
            ...
            childAccountIds.add(Trigger.new[i].Child_Account_vod__c);
        }
    }
    ...
    for (Integer i=0; i < Trigger.new.size(); i++) {
        ...
        if (Trigger.new[i].Account_vod__c != null && Trigger.new[i].Child_Account_vod__c == null) {
        ...
        } else if (Trigger.new[i].Child_Account_vod__c != null) {
			Child_Account_vod__c childAcc = childAccounts.get(Trigger.new[i].Child_Account_vod__c);
            if (childAcc != null) {
                ...
                if (childAcc.Child_Account_vod__c != null) {
                    Trigger.new[i].Account_vod__c = childAcc.Child_Account_vod__c;
                }
