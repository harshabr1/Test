trigger VOD_ACCOUNT_TEAM_MEMBER_AFTER_INSERT_UPDATE on Account_Team_Member_vod__c (after insert, after update) {
    ...
    for (Account_Team_Member_vod__c  actTeamMember : Trigger.new) {
        ...
        if (actTeamMember.Access_vod__c == null || actTeamMember.Account_Plan_vod__c == null || actTeamMember.Team_Member_vod__c == null) {
    ...
    // all members which qualified with the plan tactic share with values for downward insert
    //Set<Account_Team_Member_vod__c> accountTeamMembers = new Set<Account_Team_Member_vod__c> ();
    ...
    // add entry for account plan share object
    /*List<SObject> accountPlans = Database.query('Select Id, Name from Account_Plan_vod__c where Id in : accountPlanIds');
    List<SObject> accountPlanShareObjects = new List<SObject> ();
    for (SObject acocuntPlan: accountPlans) {
        String accountplanId = (String) acocuntPlan.get('Id');      
        List<Account_Team_Member_vod__c> planMembers = accountPlanIdTeamMemberMap.get(accountplanId);
        for (Account_Team_Member_vod__c actMember : planMembers) {
            if (actMember.Account_Plan_vod__c == accountplanId && Schema.getGlobalDescribe().get('Account_Plan_vod__Share') != null) {
                SObject accountPlanShare = Schema.getGlobalDescribe().get('Account_Plan_vod__Share').newSObject();            
                accountPlanShare.put('UserOrGroupId', actMember.Team_Member_vod__c) ;
                accountPlanShare.put('ParentId' , accountplanId);
                accountPlanShare.put('RowCause', 'Veeva_Account_Plan_Sharing_vod__c');
    ...
    // now with the account plan ids query the plan tactic object 
    /*List<SObject> planTacticShareObjects = new List<SObject> ();
    // map which has the plan tactic id along with the account team member to be used for account tactic object
    Map<Id, Account_Team_Member_vod__c> planTacticIdTeamMemberMap = new Map<Id, Account_Team_Member_vod__c> ();   
    List<Id> planTacticIds = new List<Id> ();  
    Set<Id> delPlanTacticIds = new Set<Id> ();
    Set<ID> accoutMemberIDs = new Set<ID> ();
    List<SObject> planTactics = Database.query('Select Id, Account_Plan_vod__c,Share_With_vod__c from Plan_Tactic_vod__c where Account_Plan_vod__c in : accountPlanIds');
    for (SObject planTactic : planTactics ) {
        // now check the matching conditions and create plan tactic share object for upsert
        // TODO add a check here to see if the org wide sharing for this object is set to private before we access the object otherwise it will break in orgs which does not have org wide priavte settings 
        String planTacticAccountPlan = (String) planTactic.get('Account_Plan_vod__c');     
        List<Account_Team_Member_vod__c> planMembers = accountPlanIdTeamMemberMap.get(accountplanId);
        for (Account_Team_Member_vod__c actMember : planMembers) {
            // check if the share with vod matches
            String shareWithValues = (String ) planTactic.get('Share_With_vod__c');  
            if (actMember == null || shareWithValues == null || actMember.Role_vod__c == null) {
                continue;
            }         
            if (actMember.Account_Plan_vod__c == planTacticAccountPlan  && Schema.getGlobalDescribe().get('Plan_Tactic_vod__Share') != null) {
                // if share with matches we need to add otherwise we need to delete
                String planTacticId = (String) planTactic.get('Id');
                if (shareWithValues.contains(actMember.Role_vod__c)) {         
                    // add entried to be used with account tactic object                
                    planTacticIds.add(planTacticId);
                    planTacticIdTeamMemberMap.put(planTacticId, actMember); 
                    // create share object for the plan tactic
                    SObject planTacticShareObj  = Schema.getGlobalDescribe().get('Plan_Tactic_vod__Share').newSObject();            
                    planTacticShareObj.put('UserOrGroupId', actMember.Team_Member_vod__c) ;
                    planTacticShareObj.put('ParentId' , planTacticId);
                    planTacticShareObj.put('RowCause', 'Veeva_Account_Plan_Sharing_vod__c');
                    if (actMember.Access_vod__c.equals('Read_vod')) {
                        planTacticShareObj.put('AccessLevel' ,'Read');
                  ...
                  // possible delete ids
                  accoutMemberIDs.add(actMember.Team_Member_vod__c);
                  delPlanTacticIds.add(planTacticId);            
                }           
            }
        }        
