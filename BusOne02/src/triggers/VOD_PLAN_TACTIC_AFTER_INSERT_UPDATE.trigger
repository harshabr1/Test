trigger VOD_PLAN_TACTIC_AFTER_INSERT_UPDATE on Plan_Tactic_vod__c (after insert, after update) {
    ...
    for (Account_Team_Member_vod__c member : [Select Id, Name, Account_Plan_vod__c, Team_Member_vod__c, Role_vod__c, Access_vod__c from Account_Team_Member_vod__c where Role_vod__c != null AND Access_vod__c  != null AND Account_Plan_vod__c in :accountPlanIds]) {
    ...
    for (Plan_Tactic_vod__c planTactic : Trigger.new) {
        ...
        for (Account_Team_Member_vod__c teamMember : accountTeamMembers.values()) {
            if (tacticAcctPlan == teamMember.Account_Plan_vod__c && shareWithValues.contains(teamMember.Role_vod__c)) {
